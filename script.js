...
(function freeTarot() {
  ...
  function revealCard(card, clickedEl) {
    ...
    setTimeout(() => {
      ...
      img.onerror = function() {
        ...
        this.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="180" height="290" viewBox="0 0 180 290"><rect width="180" height="290" fill="%230d0b2a"/><text x="90" y="145" text-anchor="middle" fill="%23d4a843" font-size="14" font-family="serif">' + encodeURIComponent(card.name) + '</text></svg>';
      };

      document.getElementById('ftSuitBadge').textContent = card.suit;
      document.getElementById('ftCardName').textContent = card.name;

      const kwRow = document.getElementById('ftKeywordsRow');
      kwRow.innerHTML = card.keys.map(k => `<span class="ft-keyword">${k}</span>`).join('');

      document.getElementById('ftCardMeaning').textContent = card.meaning;
  ...
  drawAgainBtn.addEventListener('click', () => {
    shuffleBtn.textContent = '✦ Shuffle & Spread the Cards';
